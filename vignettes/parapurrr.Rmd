...
parapurrr is a package to run purrr's mapping functions in parallel (i.e., incorporating multiple CPU cores instead of the default, one); It simply does it by connecting [purrr](https://cran.r-project.org/package=purrr "purrr: Functional Programming Tools") with [foreach](https://cran.r-project.org/package=foreach "foreach: Provides Foreach Looping Construct") package and its adaptors. Users are only required to add a prefix "pa\_" before their desired purrr functions (e.g. pa_map instead of map). That's it! You do not need to worry about anything else, the rest will be handled internally for you.
...
Currently, parapurrr is only published on [GitHub](https://github.com/moosa-r/parapurrr). To install the latest developmental version of parapurrr, You can use `remotes` package:
...
Broadly speaking, computations can be performed in sequential or in parallel. These terms are self-explanatory, but what we do mean here by sequential is that a single CPU core will be dedicated to performing your jobs. So, your function will be applied to your input's elements one by one, meaning that each call will have to wait for the previous call to finish (hence the term sequential). On the other hand, what we mean here by parallel is that your job will be split into multiple segments, and each segment will be sent to a separate process and then to a distinct CPU core, to be computed at the same time (hence the term parallel).

This package aims to make the latter possible by requiring the minimum effort or background knowledge from the users. As mentioned earlier, you could simply add the prefix "pa\_" before the purrr's mapping function name (e.g., pa_map instead of map). Of course, there is much more and you have control over various aspects of the parallelization job. For instance, you can choose the backend, the parallelization strategy, and the number of workers. You can do that by using arguments that are available in every parapurrr function, but they are optional; So, more on that later.

Under the hood, after splitting your job into multiple segments, parapurrr will hand over each job to the famous package foreach and its adaptor. Every available foreach adaptor in CRAN is supported; Thus, you have a variety of choices based on your running environment. The adaptor, number of CPU cores to use, along with multiple other options could be altered within each parapurrr function call; Nevertheless, each of these arguments is either optional or have a default value and you can leave them as they are.
...
```{r pid_parallel}
pid_par <- pa_map(1:2, ~Sys.getpid())
print(pid_par)
```

You can see that different PIDs have been returned. This is because to do your job in parallel, different R processes were spawned or forked. You can even confirm that the number of unique PIDs does correspond to "cores" argument. Another- perhaps more tangible- approach would be to return the time within your .f function:
...
Each foreach adaptor relies on specific architecture to perform your code in parallel. Most of the backends allow you to have different options to choose based on your system and needs. The allowed values are depended on your selected adaptor and your operation system.
...
To do your job in parallel, parapurrr will split your input into multiple segments. This splitting process is as symmetrically distributed as possible and will not consider any factors other than your input's length and available workers (cores). But you override this by explicitly telling parapurrr how to split your input. To do this, provide a list where each of its elements is a vector of integers or integer-like numbers (i.e. no decimal points) of the indexes of your input elements.
...
Note that some complications may arise when you manually provide the splitter argument:

1.  You may provide duplicated element indexes or miss some elements. In such cases, parapurrr will halt the code execution and issues an error.

2.  There may be inconsistency between your provided number of CPU cores (i.e. workers) and the length of the splitter list (which implicitly dictate the number of workers to use). In this case, parapurrr will issue a warning, ignores your provided "cores" arguments, and continues the code execution based on the splitter argument.

3.  When using `pa_map_if` or `pa_map_at`, only the elements of .x which have been selected by .p or .at respectively, will be used as the input. Thus, the splitter should correspond to the selected elements of .x not .x itself. Parapurrr will try to detect this issue and inform you about it. For example, if your input has 10 elements and 4 of them is going to be selected by .at, your splitter should contain the indexes 1 to 4.
...
3.  After finishing the execution of your code, the cluster and its relevant processes will be terminated and your environment will be cleaned.
...
x <- pa_map(1:2, ~Sys.getpid(), adaptor = NULL)
...
## Force it by running manual_register(TRUE)

This will force parapurrr functions to ignore any provided "adaptor" argument.The rest will be identical to the first way: if any backend were registered, parapurrr will use it; else, your functions will be run in sequential mode. To revert to the automatic handling, simply run manual_register(FALSE).

```{r manual_register, warning=TRUE}
manual_register(TRUE)
# We expect 2 warnings here:
x <- pa_map(1:2, ~Sys.getpid(), adaptor = "doParallel")
